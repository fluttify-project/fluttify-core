...
open class Export : FluttifyTask() {
    ...
    fun process() {
        ...
        val androidExportFile =
            "${project.projectDir}/output-project/${ext.projectName}/lib/src/android/android.export.g.dart".file()
        val iosExportFile =
            "${project.projectDir}/output-project/${ext.projectName}/lib/src/ios/ios.export.g.dart".file()
        ...
        androidDartSrcDir.iterate("dart") {
            ...
            if (relativePath.isNotBlank()) androidResult.appendLine("export '${relativePath}';")
        }
        iosDartSrcDir.iterate("dart") {
            val relativePath = it.toRelativeString(iosExportFile).removePrefix("../")
            if (relativePath.isNotBlank()) iosResult.appendLine("export '${relativePath}';")
        }

        ext.pluginDependencies.forEach { (name, _) ->
            androidResult.appendLine("export 'package:$name/$name.dart';")
            iosResult.appendLine("export 'package:$name/$name.dart';")
        }
        ...
        androidResult.appendLine("export '../facade/shared.g.dart';")
        iosResult.appendLine("export '../facade/shared.g.dart';")
