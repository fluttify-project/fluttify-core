...
import me.yohom.fluttify.tmpl.objc.common.nsvalue_struct.NSValueToStructTmpl
import me.yohom.fluttify.tmpl.objc.common.nsvalue_struct.StructToNSValueTmpl
    ...
    fun objcCallbackReturn(): String {
        return tmpl
            ...
            .replaceParagraph("#__raw_callback_result__#", method.returnType.run {
                when {
                    jsonable() -> ReturnJsonableTmpl(
                        method
                    ).objcReturnJsonable()
                    findType().isRefType() -> ReturnRefTmpl(
                        method
                    ).objcReturnRef()
                    findType().isStruct() -> ReturnStructTmpl(
                        method
                    ).objcReturnStruct()
                    else -> this
                }
