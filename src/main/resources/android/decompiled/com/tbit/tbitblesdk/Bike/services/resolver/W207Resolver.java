...
public class W207Resolver implements Resolver {
   ...
   public void resolveAll(BikeState bikeStates, byte protocolVersion, Byte[] data) {
      if(data != null && data.length != 0) {
         ...
         if(data.length >= 43) {
            controllerInfoData = (Byte[])Arrays.copyOfRange(data, 24, 43);
   ...
   public static double[] resolveLocations(double d) {
      double[] locations = new double[]{0.0D, 0.0D};

      try {
         double e = (double)((int)d);
         locations[0] = e;
         double min = d % 1.0D * 60.0D;
   ...
   private BControllerState resolveBControllerState(Byte[] data) {
      ...
      if(data != null && data.length >= 19) {
         ...
         controllerState.setChargeCount(StateUpdateHelper.byteArrayToInt(chargeData));
